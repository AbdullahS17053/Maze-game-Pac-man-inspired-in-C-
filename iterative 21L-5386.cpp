...
public:
	...
	void push(type element)//add to tail
	{
		if (tail == NULL)
		{
...
public:
	...
	void play()
	{
		...
		{
			if (checkDevil() == true || checkTres() == true || checkPortal() == true)//conditions to exit game
			{
				return;
			}
			...
			case KEY_RIGHT: //move right
				...
				break;
			case VK_KEY_Q: //quit game using 'q' key
	...
	bool checkPortal()//check to see if person lands on 'P'
	{
		...
		if (rooms[currentroom.top() - 1].map[x][y] == 'P')
		{
			...
			if (rand() % (2) == 0) // heads
			{
				while (num == currentroom.top() - 1)// change to new random room
				{
					num = rand() % (totalrooms);
				}
			...
			{
				currentroom.pop();
				if (currentroom.isEmpty())// kick if room stack empty
				{
...
{
	maze hauntedhouse;
	hauntedhouse.loadRooms();//load text files
	hauntedhouse.play();//play game
}